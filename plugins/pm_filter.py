...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("This Message is not for you. Don't worry you can send new one !", show_alert=True)
...
async def cb_handler(client: Client , query: CallbackQuery):
    #usersid = query.from_user.id
    #user_link = f'<a href="tg://user?id={usersid}">Userlink</a>'
    data = query.data
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer(url=f"https://t.me/{temp.U_NAME}?start={ident}_{file_id}")
                return
            else:button = InlineKeyboardButton('🔰 ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ 🔰', url='https://t.me/filmztube'),InlineKeyboardButton('⚜️ ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ ⚜️', url='https://t.me/+OJPH-0u_62FmZjI9')
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Do not overthink ok! just join 😒.", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        if f_caption is None:
            f_caption = f"{title}"
        await query.answer()
        button = InlineKeyboardButton('🔰 ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ 🔰', url='https://t.me/filmztube'),InlineKeyboardButton('⚜️ ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ ⚜️', url='https://t.me/+OJPH-0u_62FmZjI9')
        markup = InlineKeyboardMarkup([[button]])
        await client.send_cached_media(
            chat_id=query.from_user.id,
            file_id=file_id,
        ...
        try:
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            user_link = f'<a href="tg://user?id={user_id}">Userlink</a>'
            await client.send_message(int(user_id), f"🛋 Hello, Your requested content named `{movie}` is already available in our database! You can easily get this movie by searching it's correct name in our official group...\nSend details to Admin : \n\n❤ Thank You for the contribution", reply_markup=reply_markup)
        ...
        try:
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            user_link = f'<a href="tg://user?id={user_id}">Userlink</a>'
            await client.send_message(int(user_id), f"✅ Hello, Your requested content named `{movie}` is now available in our database! You can easily get this movie by searching it's correct name in our official group...\n\n❤ Thank You for the contribution", reply_markup=reply_markup)
            await query.edit_message_text(text=f"- __**User notified successfully sweetie...✅**__\n\n⏳**Status** : Upload done ✅.\n🪪**UserInfo** : {user_link}\n🎞**Content** : `{movie}`\n\n\n🦋", reply_markup=reply_markup_lzdv)
        # Delete callback query message
            await query.answer()
            await query.delete()
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ 2 ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://telegram.me/netflix_weblinks/11'),
        ] 
    )
    if offset != "":
    ...
    if imdb:
        ...
        cap = f"Hello, Here is what i found for your query {search}"
